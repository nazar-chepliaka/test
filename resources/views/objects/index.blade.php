    ...
    <body class="antialiased">
        ...
        <div id="instruments-container">
            ...
            {{-- 
                ...
                <svg width="24px" height="24px" viewBox="0 0 24 24" focusable="false" fill="currentColor" class=" a-s-fa-Ha-pa"><path d="M11 5.83V10h2V5.83l1.83 1.83 1.41-1.42L12 2 7.76 6.24l1.41 1.42zm6.76 1.93l-1.42 1.41L18.17 11H14v2h4.17l-1.83 1.83 1.42 1.41L22 12zM13 18.17V14h-2v4.17l-1.83-1.83-1.41 1.42L12 22l4.24-4.24-1.41-1.42zM10 13v-2H5.83l1.83-1.83-1.42-1.41L2 12l4.24 4.24 1.42-1.41L5.83 13z"></path></svg> 
            --}}
        ...
        <script>
            ...
            window.moveObject = function(e) {
                ...
                var y_diff = window.original_mouse_y - e.pageY;
                var y_diff_abs = Math.abs(y_diff);

                if (x_diff_abs <= grid_size && y_diff_abs <= grid_size) {
                    return;
                }

                if (x_diff_abs > grid_size) {
                    var r = x_diff_abs % grid_size;
                    var result_x_distance = x_diff_abs - r;

                    var curr_left_val_int = window.draggableObj.style['left'];

                    if (curr_left_val_int.length > 2) {
                        curr_left_val_int = parseInt(curr_left_val_int);
                    } else {
                        curr_left_val_int = 0;
                    }

                    var result_left_val = curr_left_val_int;

                    if (x_diff > 0) {
                        window.original_mouse_x = window.original_mouse_x - result_x_distance;
                ...
                if (y_diff_abs > grid_size) {
                    var r = y_diff_abs % grid_size;
                    var result_y_distance = y_diff_abs - r;

                    var curr_top_val = window.draggableObj.style['top'];

                    if (curr_top_val.length > 2) {
                        curr_top_val_int = parseInt(curr_top_val);
                    } else {
                        curr_top_val_int = 0;
                    }

                    var result_top_val = curr_top_val_int;
